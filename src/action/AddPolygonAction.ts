namespace action {
	...
	export class AddPolygonAction implements Action {
		readonly description: ActionKeyWord[];
		readonly layerNum: number;

		private points: Array<number>;
		private newPoint: math.Point;
		private color: string;

		constructor(layer: number, color: string, points: Array<number>, newPoint?: math.Point) {
		...
		public undo(ctx: ui.AppContextProps): void {
			...
			if (tool.AddPolygon.isAddPolygon(ctx.tool)) {
	...
	export class ExpandPolygon implements Action {
		readonly description: ActionKeyWord[];
		readonly layerNum: number;

		private polygonNum: number;
		private point: number | math.Point;

		constructor(layer: number, polygonNum: number, point: number | math.Point) {
