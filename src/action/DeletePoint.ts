namespace action {
	...
	export class DeletePoint implements Action {
		...
		private deletedLines: Array<{ line: types.Line, num: number }> = []; //also store their position in array to preserve order when undoing
		private deletedPolygons: Array<{ num: number, polygon: types.Polygon }> = [];
		private totalPolygons: number;

		constructor(layerNum: number, pointNum: number, point: math.Point) {
		...
		public do(ctx: ui.AppContextProps) {
			...
			const newLines: Array<types.Line> = [];
			...
			const newPolygons: Array<types.Polygon> = [];
			for (let i = 0; i < layer.polygons.length; i++) {
				const polygon = layer.polygons[i];
				const newPolygon: types.Polygon = { color: polygon.color, points: [] };
