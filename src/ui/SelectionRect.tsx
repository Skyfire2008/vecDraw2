namespace ui {
	...
	export const SelectionRect: React.FC<SelectionProps> = React.memo(({ svgWidth, svgHeight, gridWidth, gridHeight, pan, zoom, selection, layers, setLayers, addAction, activeLayer }) => {
		const [dims, setDims] = React.useState<types.Dimensions>(null);
		const prevDims = React.useRef<types.Dimensions>(null);
		const prevScaling = React.useRef<math.Point>(null);
		const prevLayer = React.useRef<types.LayerData>(null);
		const prevSelection = React.useRef<Set<number>>(null);
		const origPositions = React.useRef<Map<number, math.Point>>(null);

		const [isTransforming, setIsTransforming] = React.useState(false);
		const referencePoint = React.useRef<math.PointLike>(null);
		...
		if ((layers[activeLayer] != prevLayer.current || selection != prevSelection.current) && !isTransforming) {
			...
			const newDims: types.Dimensions = { left: Number.POSITIVE_INFINITY, right: Number.NEGATIVE_INFINITY, top: Number.POSITIVE_INFINITY, bottom: Number.NEGATIVE_INFINITY };
			...
			origPositions.current = new Map<number, math.Point>();
			for (const num of selection) {
		...
		const onStretchMouseDown = (newReferencePoint: math.PointLike, e: React.MouseEvent) => {
			e.bubbles = false;
			e.stopPropagation();

			referencePoint.current = newReferencePoint;
			prevDims.current = dims;
			prevScaling.current = new math.Point(1, 1);
		...
		const onOverlayMouseUp = (e: React.MouseEvent) => {
			...
			const layer = layers[activeLayer];
			const newPositions = new Map<number, math.Point>();
			for (const num of selection) {
				newPositions.set(num, layer.points[num].clone());
			}

			addAction(new action.ScalePoints(activeLayer, origPositions.current, newPositions));
		...
		const onOverlayMouseMove = (e: React.MouseEvent) => {
			...
			if (guard.current) {
				...
				const d = math.Point.subtract({ x: e.clientX, y: e.clientY }, math.Point.sum(pan, { x: rect.x, y: rect.y }));
				d.multScalar(1 / zoom);
				d.x = Math.round(d.x / gridWidth) * gridWidth;
				d.y = Math.round(d.y / gridHeight) * gridHeight;

				const scaling = new math.Point(d.x - referencePoint.current.x, d.y - referencePoint.current.y);
				...
				if (!math.Point.equals(scaling, prevScaling.current)) {
		...
		React.useEffect(() => {
			const handler = (e: KeyboardEvent) => {
				if (e.key == "Delete") {
					addAction(new action.DeleteSelection(activeLayer, selection));
				}
			};
